...
ppt_accum_plots <- ggplot(snowflow_dat_sum%>%filter(WaterYear<2025), aes(x = WaterYear, y = Total_ppt*0.001, color = site, shape=site)) +
 annotate("rect", xmin = -Inf, xmax = 2020.5, ymin = -Inf, ymax = Inf, fill = "grey80", alpha = 0.5) +
annotate("rect", xmin = 2024.5, xmax = Inf, ymin = -Inf, ymax = Inf, fill = "grey80", alpha = 0.5) +
  geom_point(size = 2.5, alpha = 0.95, position = position_dodge(width = 0.1)) +
  geom_line(position = position_dodge(width = 0.1))+
    geom_hline(yintercept = 1780.2429*0.001, col = "#054fb9", lty = 2) + 
  geom_hline(yintercept = 890.7667*0.001, col = "#DD6E42", lty = 2) + 
  theme_bw() + 
...
#    geom_vline(xintercept = 2023.5, linetype = "dotted", color = "grey30") +
  ...
  xlab(NULL) + scale_color_manual(values = c("#054fb9", "#DD6E42")) +
  theme(legend.position =  "none") +
        scale_shape_manual(name = "Shore",
                     values = c("BWL" = 15, 
                                "GBL" = 17)) +
        scale_x_continuous(breaks = seq(floor(min(snowflow_dat_sum$WaterYear)), ceiling(max(snowflow_dat_sum$WaterYear)), by = 2), labels = function(x) as.character(round(x)))
...
# ggsave("/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/Figs/Hydro_panel_long_SFS_PPT.png", plot = ppt_accum_plots, width = 7, height = 2.25, units = "in")
...
```{r, warning=F, size = 'small', echo=F, include=F, fig.width=6, fig.height=4}
# ggsave("/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/Figs/Hydro_panel_long.png", plot = combined_plot, width = 8.5, height = 5, units = "in")
